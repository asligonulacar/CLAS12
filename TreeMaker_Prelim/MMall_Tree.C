...
for(int j=23; j<28; j++){
  ...
  //TLorentzVector beam(0,0,10.6041,10.6041);
  ...
  TFile tree_file("hipo_tree.root","recreate");
...
//////////////////////////////////
      ...
      if(TMath::RadToDeg()*el.Theta() > 2.5 && TMath::RadToDeg()*el.Theta() < 4.5){
        ...
        //miss_all_corrected = beam + target - el_new - pip - pim - pr;

        //hmiss_all_corrected->Fill(miss_all_corrected.M2());

        miss_all = beam + target - el - pip - pim - pr;
        miss_all_m2.push_back(miss_all.M2());
        std::cout<<miss_all_m2[0]<<endl;
